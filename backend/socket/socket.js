...
const io = new Server(server, {
  ...
  cors: {
    origin: ["https://chat-app-mern-dh70.onrender.com"],
    methods: ["GET", "POST"],
  },
});
...
io.on("connection", (socket) => {
  ...
  if(userId !== 'undefined'){        
    if(!userSocketMap[userId]) {                                    // if userId is not undefined then only we will store the user socket id into the userSocketMap object 
      userSocketMap[userId] = [];
    }             
    userSocketMap[userId].push(socket.id) ;                              // this code store user in the object like this -> {userId: socketId}
  ...
  socket.on("disconnect", () => {
    ...
    if(userSocketMap[userId]) {
      ...
      io.emit("getOnlineUsers", Object.keys(userSocketMap));       // This line is used to delete the user from the userSocketMap object (meaning deleting the user from the online user list) when user disconnects from the server. 
    }                                                              // Since we are deleting the offline user so we have to reload the online user list to the clients so that they can see the updated online user list.                                                                    
  });
});
